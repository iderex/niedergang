...
Record 0003 fixes that breakup is an input assumption rather than a prediction,
so the tool is told when the parent comes apart and does not compute it. That
...
rather than at the assumption. It costs accuracy on any object whose real
breakup was a sequence rather than an event.

Take a sequence rather than an altitude, releasing different parts of the
...
guess with more parameters than the single number, which is worse rather than
better because it looks more careful.
...
an operator needs rather than leaving it to be inferred from its absence in the
...
both derived from impact points and tracking data rather than assumed.
...
km as a default rather than as a value, per record 0010, and the output says the
...
78 km rather than either reconstructed value, because the reconstructions are
the evidence the convention rests on rather than competing candidates, and
...
Making it an input rather than a constant is what turns the assumption into
...
Recording it as a default rather than as a value is the same argument record
...
it except a case whose breakup altitude was measured rather than reconstructed.
...
measured a sequence rather than a point: solar arrays separating near 86 km,
...
what is possible rather than about what is right.
...
and it is owed rather than known, so the argument for 78 km over any other
...
its own and the answer is a sampled altitude under record 0007 rather than a
different fixed one. The measurement is possible as soon as issue #26, issue #40
and issue #62 exist, and no figure is guessed in advance of it.

A breakup altitude measured for a spacecraft rather than reconstructed from
impact points or observed on a cargo vehicle.
...
the altitude one trigger among several rather than the trigger, and this record
...
which would argue for a per-class default rather than one number.
...
Record 0003, which fixes breakup as an assumption the tool is told rather than
computes, and which this record supplies the number for. Record 0010, which
